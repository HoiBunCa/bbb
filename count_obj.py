        ...
        for i, det in enumerate(pred):  # detections per image
            ...
            if det is not None and len(det):
                ...
                if len(outputs) == len(det):
                    ...
                    output_classes_arr = np.array(output_classes)
                    # print("output_classes", output_classes_arr)

                # for i in output_classes:
                    append_count = count_obj(output_classes)
                    print("append_count", append_count)
                    if len(append_count) > 0:
                        bbox_xyxy = append_count[:, :4]
                        identities = append_count[:, -1]
